...
import { User, ShieldCheck, CreditCard, Settings, GraduationCap, Phone, Mail, Camera, QrCode, LogOut, Loader2, Edit3, X, Trash2 } from 'lucide-react';
...
export const ProfileView: React.FC<ProfileViewProps> = ({ onOpenPlans }) => {
  ...
  const [removePicError, setRemovePicError] = useState<string | null>(null);

  const handleRemoveProfilePicture = async () => {
    try {
      setIsRemovingPic(true);
      setRemovePicError(null);

      const imageUrl = profile?.image_url || profile?.avatar_url || '';

      if (imageUrl && imageUrl.includes('/storage/v1/object/')) {
        try {
          ...
          if (pathParts.length > 1) {
            ...
            if (firstSlashIndex !== -1) {
              ...
              if (bucketName && filePath) {
                console.log(`Deleting storage file from bucket: ${bucketName}, path: ${filePath}`);
                await supabase.storage.from(bucketName).remove([filePath]);
              }
            }
          }
        } catch (storageErr) {
          console.error('Failed to clean up Supabase storage file:', storageErr);
        }
      }
      ...
      setRemovePicError(err?.message || 'Failed to remove profile picture.');
    } finally {
                ...
                {(profile?.image_url || profile?.avatar_url) && (
                  <div className="mt-2 flex justify-start">
                    <button
                      ...
                      className="text-rose-600 hover:text-rose-700 font-semibold text-xs flex items-center gap-1.5 transition-colors cursor-pointer"
                    >
